...
&& $Mongodump --archive=$bak_dir/${date_time}-backend.gz --gzip -u root -p YXTxsaj22WSJ7wTG  --authenticationDatabase admin -d backend  &>>$bak_dir/${date_time}-bak.log \
...
# gs restore 27017 
# gzip -d archivefile
# mongorestore --archive=${date_time} --username root --password YXTxsaj22WSJ7wTG
