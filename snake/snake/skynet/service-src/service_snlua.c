...
#ifdef OPEN_MY_ALLOC
	l->L = lua_newstate(snlua_skynet_lalloc, snlua_allocator_new());
#else
...
#ifdef OPEN_MY_ALLOC
	lua_Alloc loc = lua_getallocf(l->L, &ud);
	assert(loc==snlua_skynet_lalloc);
	lua_close(l->L);
	snlua_allocator_delete(ud);
#else
	lua_close(l->L);
#endif
