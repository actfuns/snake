...
local mymeasure = require "mymeasure"

local coroutine_resume = mymeasure.resume
local coroutine_yield = mymeasure.yield
...
local coroutine_trace = setmetatable({}, { __mode = "kv" })
...
function skynet.is_open_measure()
	return is_open_measure
end

function skynet.open_measure()
	if not is_open_measure then
		for v, _ in pairs(coroutine_trace) do
			mymeasure.open(v)
		end
		is_open_measure = true
	end
end

function skynet.close_measure()
	if is_open_measure then
		for v, _ in pairs(coroutine_trace) do
			mymeasure.close(v)
		end
		is_open_measure = false
	end
end
...
function skynet.servicekey(name)
	if type(name) == "string" then
		return c.service_key(name)
	else
		return name
	end
end
...
function skynet.change_gc_size(i)
	local prev = step_gc_size
	step_gc_size = i
	return prev
end
...
function skynet.dispatch_message(...)
	...
	collectgarbage("step", step_gc_size)
...
function skynet.priority(b)
	if b then
		c.command("PRIORITY", "1")
	else
		c.command("PRIORITY", "0")
	end
end
