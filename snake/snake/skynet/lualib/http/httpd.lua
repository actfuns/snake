...
local function readall(readbytes, bodylimit, debug)
	local tmpline = {}
	if debug then
		debug[1] = "recvheader"
	end
	...
	assert(method and url and httpver, string.format("httpd readall err method:%s, url:%s, httpver:%s", method, url, httpver))
