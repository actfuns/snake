local oAnleiTask = require("common/task/task_anlei")
local oFightTask = require("common/task/task_fight")
local oFindItemTask = require("common/task/task_finditem")
local oFindPersonTask = require("common/task/task_findperson")
local oFindSummonTask = require("common/task/task_findsummon")
local oPickTask = require("common/task/task_pick")
local oUseItemTask = require("common/task/task_useitem")
local taskdefines = require("common/task/taskdefines")
...
function CTaskMgr:DelTask(mArgs)
    if mArgs.taskid == self.m_iCurTaskId then
        ...
        local iDelete = nil
        for idx, mTask in ipairs(self.m_mTaskInfoTbl or {}) do
            if mArgs.taskid == mTask.taskid then
                iDelete = idx
            end
        end
        if iDelete then
            table.remove(self.m_mTaskInfoTbl, iDelete)
        end
    end
