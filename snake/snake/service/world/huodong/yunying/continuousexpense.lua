...
function CHuodong:MergeFrom(mFromData)
    if not mFromData or not next(mFromData) then
        return false, "huodong continuousexpense without data"
    end
    self:Dirty()
...
function CHuodong:RegisterHD(mInfo, bClose)
    if bClose then
        ...
        local bIsOpen = global.oToolMgr:IsSysOpen("CONTINUOUS_EXPENSE") 
        if not bIsOpen then
...
function CHuodong:RefreshAllPlayer()
    ...
    global.oToolMgr:ExecuteList(lAllOnlinePid, 100, 1000, 0, "ContinuousExpenseRefresh", func)
end
...
function CHuodong:UpdateRewardState(iPid)
    ...
    local iTodayGoldCoin = oPlayer.m_oTodayMorning:Query("today_expense_goldcoin", 0)
...
function CHuodong:CheckReward(oPlayer)
    ...
    -- local bIsOpen = global.oToolMgr:IsSysOpen("CONTINUOUS_EXPENSE", oPlayer, true) 
...
function CHuodong:GS2CGameStart(oPlayer)
    ...
    oPlayer:Send("GS2CContinuousExpenseStart",mNet)
end
...
function CHuodong:GS2CGameReward(oPlayer)
    local mNet = {
        ...
        curgoldcoin = oPlayer.m_oTodayMorning:Query("today_expense_goldcoin", 0),
    ...
    oPlayer:Send("GS2CContinuousExpenseReward", mNet)
end
...
function CHuodong:C2GSContinuousExpenseSetChoice(oPlayer, mData)
    if self:GetGameState() ~= GAME_START then return end
...
function CHuodong:C2GSContinuousExpenseReward(oPlayer, iDay)
    if self:GetGameState() ~= GAME_START then return end
...
function CHuodong:C2GSContinuousExpenseTotalReward(oPlayer, iDay)
    if self:GetGameState() ~= GAME_START then return end
...
function CHuodong:LogState()
    ...
    record.log_db("huodong", "continuousexpense_state", mLogData)
end
...
function CHuodong:LogReward(oPlayer, iDay, bIsTotal)
    ...
    local sStr = bIsTotal and "continuousexpense_totalreward" or "continuousexpense_reward"
...
function CHuodong:TestOp(iFlag, mArgs)
    ...
    local mCommand={
        "100 指令查看",
        "101 活动开始（1:新 2:老）\nhuodongop continuousexpense 101  {mode=1}",
        "102 活动结束\nhuodongop continuousexpense 102",
        "104 调整到第几天\nhuodongop continuousexpense 103 {day = 2}",
    }
    if iFlag == 100 then
    ...
    elseif iFlag == 103 then
        ...
        if self:GetGameState() == GAME_START then
            oPlayer.m_oTodayMorning:Delete("today_expense_goldcoin")
            self:UpdateState(oPlayer:GetPid())
            self:GS2CGameStart(oPlayer)
            self:GS2CGameReward(oPlayer)
        end
