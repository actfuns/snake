...
function CToday:Delete(key)
    if not self.m_mKeepList[key] and not self.m_mData[key] then
        return
    end
    self:Dirty()
...
function CThisTemp:QuerySecond(key)
    local iEndTime = 0
    if self.m_mKeepList[key] then
        iEndTime = self.m_mKeepList[key]
    end
    return iEndTime
end
...
function CThisTemp:Reset(key, value, iSecs)
    self:Delete(key)
    self:Set(key, value, iSecs)
end
...
function CSeveralDay:Delete(key)
    if not self.m_mDayList[key] and not self.m_mData[key] then
        return
    end
    self:Dirty()
...
function CTimeCtrl:Release()
    for _,oSaveObj in pairs(self.m_List) do
