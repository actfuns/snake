...
function CBuff:OnBoutEnd(oAction,oBuffMgr)
    ...
    if oAttack then
        local oPerform = oAttack:GetPerform(8004)
        if oPerform then
            local sExtArg = oPerform:ExtArg()
            local mEnv = oPerform:SkillFormulaEnv(oAttack, oAction)
            local mExtArg = formula_string(sExtArg, mEnv)
            iHp = iHp + mExtArg.sub_hp
            iMp = iMp + mExtArg.sub_mp
        end
    end
    ...
    iMp = math.min(iMp, oAction:GetMp())
