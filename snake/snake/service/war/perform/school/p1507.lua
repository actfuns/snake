...
function CPerform:TruePerform(oAttack,oVictim,iRatio)
    ...
    local oBuff = oBuffMgr:HasBuff(121)
    if oBuff then
        return
    end
...
function CPerform:HitRatio(oAttack, oVictim)
    local iRatio = super(CPerform).HitRatio(self, oAttack, oVictim)
    if oVictim and oVictim:IsPlayer() then
        iRatio = math.floor(iRatio / 2)
    end
    return iRatio
end
