...
function CPayMgr:PayCallback(mCbInfo)
    local iOrderId = tonumber(mCbInfo.appOrderId)
    local iPid = tonumber(mCbInfo.playerId)
    local iSign = mCbInfo.sign
    mCbInfo.sign = nil
    local iLocalSign = global.oDemiSdk:Sign(mCbInfo)
    if iLocalSign ~= iSign then
        record.unmove("pay", "payerror", {type="wrong_sign", order_id = iOrderId, pid = iPid, server = mCbInfo.serverId, cbdata = mCbInfo, request={}})
        record.error("PayCallback %s sign error: %s %s", iOrderId, iLocalSign, iSign)
        return
    end

    local mRequest = self:QueryRequestOrder(iOrderId)
    if not mRequest then
        record.unmove("pay", "payerror", {type="no_request", order_id = iOrderId, pid = iPid, server = mCbInfo.serverId, cbdata = mCbInfo, request = mRequest})
        record.error("PayCallback no such request order %s", iOrderId)
        return
    end
    local sExt = mCbInfo.callbackInfo --透传数据(ext数据)
    local mExt = cjson.decode(sExt)
    if tonumber(mCbInfo.amount) ~= mRequest.cent then
        record.unmove("pay", "payerror", {type="wrong_amount", order_id = iOrderId, pid = iPid, server = mCbInfo.serverId, cbdata = mCbInfo, request = mRequest})
        record.error("PayCallback %s pay amount not equal: %s %s", iOrderId, mCbInfo.amount, mRequest.cent)
        return
    end
    local mOrder = {
        request_id = tonumber(mCbInfo.requestId), --请求时间戳
        channel_uuid = mCbInfo.accountId, --渠道账号唯一标识
        demi_channel = tonumber(mCbInfo.channelId), --渠道在德米的编号
        pid = iPid,
        serverkey = mCbInfo.serverId,
        orderid_ch = mCbInfo.orderId, --渠道交易订单号
        amount = tonumber(mCbInfo.amount), --充值金额（分）
        orderid = iOrderId, --游戏应用订单号
        sandbox = mCbInfo.sandbox, --是否沙箱环境
        account = mExt.account,
        product_key = mRequest.productId,
        product_amount = tonumber(mRequest.amount),
        request = mRequest,
        deal = 0,
        ext = mExt,
    }

    local bOk, sErr = self:InsertOrder(mOrder)
    if not bOk then
        record.error("PayCallback insert error %s, order %s", sErr, extend.Table.serialize(mOrder))
        return
    end
    self:DeleteRequestOrder(iOrderId)
