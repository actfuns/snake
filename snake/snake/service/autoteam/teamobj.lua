...
function CTeam:Leave(pid)
    local oMem = self.m_mMember[pid]
    if oMem then
...
function CTeam:AddMem2BlackList(pid, iEndTimeStamp)
    self.m_mBlackList[pid] = iEndTimeStamp
end
...
function CTeam:StartAutoMatch()
    ...
    self:AddTimeCb("TeamAutoMatch", 3 * 1000, function () 
        _AutoMatching(iTargetID,iTeamID)
    end)
end
...
function CTeam:AutoMatching()
    self:DelTimeCb("TeamAutoMatch")
    local iNow = get_time()
    local iEndTime = self.m_iMatchStartTime + (24 * 60*60)
    if iNow < iEndTime then
...
function CTeam:AutoMatch()
        if not self:IsTeamAllowed() then return end
        ...
        if next(lMatchPlayers) then
            ...
            for pid, oMem in pairs(lMatchPlayers) do
                ...
                if iGrade >= iMinGrade and iGrade <= iMaxGrade and oMem:IsTeamAllowed() then
                    ...
                    if iCountWeight < iMinWeight then
...
function CTeam:IsTeamAllowed()
    local oLeader = self:GetMember(self.m_iLeader)
    if oLeader then
        return oLeader:IsTeamAllowed()
    else
        return false
    end
end

function _AutoMatching(iTargetID,iTeamID)
    local oTeam = global.oTargetMgr:GetTargetTeam(iTargetID,iTeamID)
