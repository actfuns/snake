...
function SaveWarVideoIdCounter(mCond, mData)
    local oGameDb = global.oGameDb
    oGameDb:Update(sIdCounterTableName, {type = "warvideo"}, {["$set"]={data = mData.data}}, true)
end

function LoadWarVideoIdCounter(mCond, mData)
    local oGameDb = global.oGameDb
    local m = oGameDb:FindOne(sIdCounterTableName, {type = "warvideo"}, {data = true})
    m = m or {}
    return {
        data = m.data,
    }
