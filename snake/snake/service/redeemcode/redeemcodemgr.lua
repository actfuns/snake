...
function CRedeemCodeMgr:SaveRedeemRuleInfo(mData)
    if not mData then 
        record.warning("CRedeemCodeMgr:SaveRedeemRuleInfo error no data")
        return {errcode=1, errmsg="no data"} 
    end

    local sRewards = mData["rewards"]
    local lRewards = split_string(sRewards, ";")
    if #lRewards <= 0 then
        record.warning("CRedeemCodeMgr:SaveRedeemRuleInfo error no rewards") 
        return {errcode=1, errmsg="no rewards"} 
    end
    for _,sItem in pairs(lRewards) do
        local lArgs = split_string(sItem, ",", tonumber)
        if #lArgs ~= 4 then
            record.warning("CRedeemCodeMgr:SaveRedeemRuleInfo error rewards error") 
            return {errcode=1, errmsg="rewards error"..sItem} 
        end
    end
    ...
    mRule["mail_title"] = mData["mail_title"]
    mRule["mail_context"] = mData["mail_context"]
    mRule["rewards"] = sRewards
...
function CRedeemCodeMgr:TrueMakeRedeemCode(iRedeem, iBatch, iAmount, func)
    ...
    if #mSaveData <= 0 then
        record.error(string.format("make redeemcode error redeem:%d, batch:%d, amount:%d", iRedeem, iBatch, iAmount))
        return
    end
...
function CRedeemCodeMgr:_RandomRedeemCode(iRedeem, iBatch)
    ...
    sCode = sCode .. string.format('%05d', iBatch)
    return sCode
end
...
function CRedeemCodeMgr:GetRedeemCodeByBatchId(iBatch)
    ...
    while mResult:hasNext() do
        ...
        table.insert(lRet, {code = mCode.code})
    end
    mongoop.ChangeAfterLoad(lRet)
    return lRet
end

function CRedeemCodeMgr:SearchRedeemCode(sCode)
    local mCode = self:GetDB():FindOne(sRedeemCodeTable, {code = sCode}, {code=true, batch_id=true,use_cnt=true, role_list=true})
    mongoop.ChangeAfterLoad(mCode)
    if not mCode or not mCode["batch_id"] then return end
    ...
    if not mBatch or not mBatch["redeem_id"] then return end

    local iRedeem = mBatch["redeem_id"]
    local mRedeem = self:GetDB():FindOne(sRedeemRuleTable, {redeem_id = iRedeem})
    if not mRedeem then return end
    mBatch["_id"] = nil
    mBatch["code"] = sCode
    mBatch["role_list"] = mCode["role_list"]
    mBatch["redeem_name"] = mRedeem["redeem_name"]
    return mBatch
end
...
function CRedeemCodeMgr:_TrueUseRedeemCode(mRedeem, mBatch, mCode, mArgs)
    ...
    local mCode = self:GetDB():FindAndModify(sRedeemCodeTable, {query = {code = sCode}, update = {["$inc"] = {use_cnt = 1}}, new = 1})
    mongoop.ChangeAfterLoad(mCode)
    if mCode["use_cnt"] > mRedeem["code_redeem_cnt"] then
        return {errcode = 6, errmsg = "redeem code used", data = mRedeem}
    end

    local sTime = get_time_format_str(get_time(), "%Y-%m-%d %H:%M:%S")
    self:GetDB():Update(sRedeemCodeTable, {code = sCode}, {["$set"] = {["role_list."..iPid] = sTime}}, true)
    self:GetDB():Update(sRedeemLogTable, {pid = iPid, redeem_id = iRedeem}, {["$set"] = {["code_list."..sCode] = 1}}, true)
