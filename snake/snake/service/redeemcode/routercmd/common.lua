...
function ForwardCmd.SaveRedeemRuleInfo(mRecord, mData, fCallBack)
    ...
    if br then
        ...
        fCallBack({errcode=1, errmsg="do error"})
    end
end
...
function ForwardCmd.SearchRedeemCode(mRecord, mData, fCallBack)
    local sCode = mData["code"]
    local oRedeemMgr = global.oRedeemCodeMgr
    local br, m = safe_call(oRedeemMgr.SearchRedeemCode, oRedeemMgr, sCode)
    if br then
        fCallBack({errcode=0, data=m})
    else
        fCallBack({errcode=1})
