...
function CRank:PushDataToRank(mData)
    ...
    if oRank then
        ...
        oRank = NewRankUnit(iSchool, "grade_" .. iSchool)
...
function CRank:Load(mData)
    ...
    local lRankList = mData.rank_list or {}
...
function CRankUnit:GenRankUnit(mData)
    return db_key(mData.pid), {mData.grade, mData.exp, mData.time, mData.pid, mData.name}
end
